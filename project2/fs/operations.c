...
static int number_open_files = 0;
static pthread_cond_t cond_open_file;
static pthread_cond_t cond_flag_open;
static pthread_mutex_t mutex_open_file;
static pthread_mutex_t mutex_flag_open;
...
int tfs_init() {
    ...
    if (pthread_mutex_init(&mutex_open_file, 0) != 0)
        return -1;

    if (pthread_mutex_init(&mutex_flag_open, 0) != 0)
        return -1;
    
    if (pthread_cond_init(&cond_flag_open,0) != 0)
        return -1;

    if (pthread_cond_init(&cond_open_file,0) != 0)
        return -1;

    
    
    pthread_mutex_lock(&mutex_flag_open);
    flag_open = 1;
    pthread_mutex_unlock(&mutex_flag_open);

    pthread_cond_signal(&cond_flag_open);
...
int tfs_destroy_after_all_closed() {
    ...
    while (!number_open_files == 0){
        pthread_cond_wait(&cond_open_file,&mutex_open_file);
    }
...
int tfs_open(char const *name, int flags) {
    while (!flag_open != 0){
        pthread_cond_wait(&cond_flag_open,&mutex_flag_open);
    }
...
int tfs_close(int fhandle) {
    ...
    number_open_files--;
    pthread_mutex_unlock(&mutex_open_file);
    pthread_cond_signal(&cond_open_file);
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;
